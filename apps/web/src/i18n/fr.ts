/**
 * Chaînes de l'interface (G2 : aucun texte en dur hors fichiers i18n).
 * Structure prête pour d'autres langues : dupliquer ce fichier et brancher
 * une sélection de langue quand le besoin arrivera.
 */
export const fr = {
  app: {
    name: 'Trackly',
    tagline: 'Ta bibliothèque de jeux, séries, films et livres',
  },
  notFound: {
    title: 'Page introuvable',
    code: '404',
    message: 'Cette page n’existe pas ou a été déplacée.',
    backHome: 'Retour à l’accueil',
  },
  deletionCancel: {
    title: 'Annuler la suppression de ton compte',
    pending: 'Annulation en cours…',
    success: 'C’est annulé. Ton compte Trackly et tes données sont conservés.',
    error:
      'Ce lien d’annulation n’est plus valable. Soit la suppression a déjà été annulée, soit le délai est écoulé.',
    toLogin: 'Aller à la connexion',
  },
  network: {
    offline: 'Hors connexion — tu consultes les dernières données chargées.',
    offlineShort: 'Hors connexion',
    backOnline: 'Connexion rétablie.',
  },
  theme: {
    toLight: 'Passer en mode clair',
    toDark: 'Passer en mode sombre',
    light: 'Clair',
    dark: 'Sombre',
  },
  fields: {
    email: 'Adresse e-mail',
    password: 'Mot de passe',
    newPassword: 'Nouveau mot de passe',
    displayName: 'Pseudo',
    rememberMe: 'Rester connecté',
  },
  auth: {
    loginTitle: 'Connexion',
    loginAction: 'Se connecter',
    loginPending: 'Connexion…',
    registerTitle: 'Créer un compte',
    registerAction: 'Créer mon compte',
    registerPending: 'Création…',
    logoutAction: 'Se déconnecter',
    forgotLink: 'Mot de passe oublié ?',
    noAccount: 'Pas encore de compte ?',
    registerLink: 'Inscris-toi',
    hasAccount: 'Déjà un compte ?',
    loginLink: 'Connecte-toi',
    forgotTitle: 'Mot de passe oublié',
    forgotHint:
      'Indique ton adresse e-mail : si un compte existe, tu recevras un lien de réinitialisation valable 1 heure.',
    forgotAction: 'Envoyer le lien',
    forgotPending: 'Envoi…',
    forgotDone:
      'Demande envoyée. Si un compte existe avec cette adresse, un e-mail est en route — pense à vérifier tes spams.',
    resetTitle: 'Nouveau mot de passe',
    resetAction: 'Changer le mot de passe',
    resetPending: 'Changement…',
    resetDone: 'Mot de passe changé. Tu peux te connecter.',
    resetMissingToken: 'Lien incomplet — ouvre le lien reçu par e-mail.',
    genericError: 'Une erreur est survenue. Réessaie dans un instant.',
  },
  passwordStrength: {
    label: 'Robustesse',
    levels: ['Trop court', 'Faible', 'Correct', 'Bon', 'Excellent'],
  },
  home: {
    eyebrow: 'Ton carnet de temps',
    welcome: 'Bonjour',
    lotDone: 'Ta bibliothèque est ouverte : ajoute, suis, reprends.',
    nextUp: 'Prochaine étape : le budget temps et le tableau de bord (Lot 4).',
    searchCta: 'Rechercher un jeu, un film, une série…',
    apiChecking: "Vérification de l'API…",
    apiOk: 'API opérationnelle',
    apiDown: 'API injoignable',
  },
  nav: {
    home: 'Accueil',
    library: 'Bibliothèque',
    search: 'Recherche',
  },
  account: {
    title: 'Mon compte',
    link: 'Mon compte',
    memberSince: 'Membre depuis le',
    exportTitle: 'Mes données (RGPD)',
    exportHint:
      'Télécharge tout ce que Trackly connaît de toi : profil, bibliothèque, progression, journal, visionnages, durées modifiées. Format JSON lisible.',
    exportAction: 'Télécharger mes données',
    exportPending: 'Préparation…',
    exportError: 'Export impossible pour le moment. Réessaie dans un instant.',
    dangerTitle: 'Supprimer mon compte',
    dangerHint:
      'Ton compte, ta bibliothèque, ta progression et ton historique seront effacés. Tu disposes de 7 jours pour changer d’avis : passé ce délai, l’effacement est définitif et rien n’est récupérable. Pense à exporter tes données avant.',
    dangerPasswordLabel: 'Confirme avec ton mot de passe',
    dangerAction: 'Programmer la suppression de mon compte',
    dangerPending: 'Enregistrement…',
    dangerConfirm:
      'Ton compte sera effacé dans 7 jours. Tu recevras un e-mail permettant d’annuler jusqu’au dernier moment. Continuer ?',
    // Suppression programmée (A5)
    scheduledTitle: 'Suppression programmée',
    scheduledOn: 'Ton compte et toutes tes données seront effacés le',
    scheduledMail:
      'Un e-mail t’a été envoyé : il contient un lien d’annulation utilisable même sans te connecter.',
    cancelAction: 'Annuler la suppression',
    cancelPending: 'Annulation…',
    cancelError: 'Annulation impossible pour le moment. Réessaie dans un instant.',
    cancelled: 'Suppression annulée. Ton compte est conservé.',
  },
  search: {
    title: 'Recherche',
    placeholder: 'Elden Ring, Dune, Severance…',
    typeAll: 'Tous',
    typeGame: 'Jeux',
    typeFilm: 'Films',
    typeSeries: 'Séries',
    typeBook: 'Livres',
    minChars: 'Saisis au moins 2 caractères pour lancer la recherche.',
    empty: 'Aucun résultat — essaie une autre orthographe ou un autre titre.',
    degraded: 'Certaines sources sont momentanément indisponibles : résultats partiels.',
    error: 'La recherche a échoué. Réessaie dans un instant.',
  },
  media: {
    typeLabel: { game: 'Jeu', film: 'Film', series: 'Série', book: 'Livre' },
    authors: 'Auteur',
    pages: 'Pages',
    firstPublish: 'Première publication',
    descriptionEnNote: 'Description en anglais (source Open Library, non traduite).',
    releaseDate: 'Sortie',
    genres: 'Genres',
    platforms: 'Plateformes',
    developers: 'Développeur',
    publishers: 'Éditeur',
    runtime: 'Durée',
    rating: 'Note',
    seasons: 'Saisons',
    episodes: 'épisodes',
    status: 'Statut',
    summaryEnNote: 'Résumé en anglais (source IGDB, non traduit).',
    ttbTitle: 'Temps pour terminer',
    ttbMain: 'Histoire principale',
    ttbMainExtra: 'Histoire + annexes',
    ttbCompletionist: '100 %',
    ttbUnknown: 'Durée inconnue',
    ttbSubmissions: 'contributions',
    ttbSource: 'Données communautaires IGDB — modifiables depuis ta bibliothèque.',
    attributionTmdb: 'Ce produit utilise l’API TMDB mais n’est ni approuvé ni certifié par TMDB.',
    attributionIgdb: 'Données jeux fournies par IGDB.com.',
    attributionOpenLibrary: 'Données livres fournies par Open Library.',
    notFound: 'Contenu introuvable.',
    backToSearch: '← Retour à la recherche',
  },
  library: {
    title: 'Bibliothèque',
    backToLibrary: '← Retour à la bibliothèque',
    // Un écran vide est une invitation à agir, pas un constat : un titre, ce que
    // ça apportera, et l'action. Découpé en trois pour pouvoir le mettre en page.
    emptyTitle: 'Ta bibliothèque est vide',
    emptyHint:
      'Ajoute un premier jeu, une série, un film ou un livre : Trackly calcule le temps qu’il te reste dessus.',
    emptyCta: 'Chercher une œuvre',
    emptyFiltered: 'Rien ne correspond à ces filtres.',
    emptyFilteredCta: 'Effacer les filtres',
    error: 'Impossible de charger ta bibliothèque. Réessaie dans un instant.',
    add: 'Ajouter',
    addPending: 'Ajout…',
    added: 'Ajouté à ta bibliothèque',
    alreadyInLibrary: 'Déjà dans ta bibliothèque',
    openEntry: 'Ouvrir ma fiche →',
    quickAdd: 'Ajouter à ma bibliothèque',
    addAs: 'Ajouter comme',
    platformLabel: 'Plateforme',
    platformPlaceholder: 'Choisir une plateforme…',
    platformOther: 'Autre…',
    platformCustomPlaceholder: 'Nom de la plateforme',
    remove: 'Retirer de la bibliothèque',
    removeConfirm: 'Retirer ce titre et tout son suivi de ta bibliothèque ?',
    removeConfirmAction: 'Confirmer le retrait',
    save: 'Enregistrer',
    savePending: 'Enregistrement…',
    saved: 'Enregistré ✓',
    gameStatus: {
      WISHLIST: 'Envie',
      BACKLOG: 'Backlog',
      PLAYING: 'En cours',
      PAUSED: 'En pause',
      FINISHED: 'Terminé',
      COMPLETED: '100 %',
      DROPPED: 'Abandonné',
    },
    seriesStatus: {
      TO_WATCH: 'À voir',
      WATCHING: 'En cours',
      PAUSED: 'En pause',
      FINISHED: 'Terminée',
      DROPPED: 'Abandonnée',
    },
    filmStatus: {
      TO_WATCH: 'À voir',
      SEEN: 'Vu',
      DISLIKED: 'Pas apprécié',
      DROPPED: 'Abandonné',
      REJECTED: 'Proposé, refusé',
    },
    bookStatus: {
      TO_READ: 'À lire',
      READING: 'En cours',
      PAUSED: 'En pause',
      FINISHED: 'Terminé',
      DROPPED: 'Abandonné',
    },
    completionTarget: {
      MAIN: 'Histoire',
      MAIN_EXTRA: 'Histoire + annexes',
      COMPLETIONIST: '100 %',
    },
    filters: {
      all: 'Tous',
      games: 'Jeux',
      series: 'Séries',
      films: 'Films',
      books: 'Livres',
      favorites: 'Favoris',
      anyStatus: 'Tous les statuts',
      anyGenre: 'Tous les genres',
      anyPlatform: 'Toutes les plateformes',
      sort: 'Trier',
      sortRecent: 'Modifié récemment',
      sortTitle: 'Titre A → Z',
    },
    counts: {
      episodes: 'épisodes vus',
      hours: 'h de jeu',
      work: 'œuvre',
      works: 'œuvres',
    },
    opinion: {
      title: 'Mon avis',
      rating: 'Note',
      noRating: 'Sans note',
      favorite: 'Favori',
      review: 'Critique',
      notes: 'Notes personnelles',
    },
    ownership: {
      title: 'Possessions et progression',
      addTitle: 'Ajouter une plateforme',
      addAction: '+ Plateforme',
      status: 'Statut',
      hours: 'Heures jouées',
      progress: 'Progression (%)',
      nextObjective: 'Prochain objectif',
      resumeNote: 'Note de reprise',
      resumeNoteHint: 'Où reprendre, quoi faire — ta mémoire entre deux sessions.',
      completionTarget: 'Objectif de complétion',
      journalNote: 'Note pour le journal (facultatif)',
      moreDetails: 'Dates et trophées',
      purchaseDate: 'Acheté le',
      startedAt: 'Commencé le',
      finishedAt: 'Fini le',
      lastPlayedAt: 'Dernière session',
      trophies: 'Trophées / succès',
      trophiesOf: 'sur',
      delete: 'Retirer cette plateforme',
      deleteConfirm: 'Retirer cette possession et sa progression ?',
      deleteConfirmAction: 'Confirmer',
    },
    durations: {
      title: 'Temps pour terminer',
      edit: 'Modifier les durées',
      cancel: 'Annuler',
      hint: 'En heures. Laisser vide pour revenir à la valeur automatique.',
      provenance: { auto: 'auto', manual: 'saisi', overridden: 'modifié' },
      provenanceLegend:
        'auto = IGDB · saisi = ta valeur (IGDB muet) · modifié = ta valeur remplace IGDB',
    },
    journal: {
      title: 'Journal de progression',
      empty: 'Chaque mise à jour de ta progression s’inscrira ici.',
    },
    series: {
      progress: 'Progression',
      season: 'Saison',
      markAll: 'Tout marquer',
      unmarkAll: 'Tout démarquer',
      notAired: 'à venir',
      episodesError: 'Impossible de charger les épisodes.',
      watchedOn: 'vu le',
    },
    film: {
      watchedAt: 'Vu le',
      rewatch: 'Envie de revoir',
      watchedWith: 'Vu avec',
    },
    book: {
      progressTitle: 'Ma lecture',
      currentPage: 'Page actuelle',
      progressPercent: 'Progression (%)',
      progressPercentHint: 'Pour les liseuses : si renseigné, le % prime sur la page.',
      resumeNote: 'Note de reprise',
      resumeNoteHint: 'Chapitre, contexte — ta mémoire entre deux sessions.',
      minutesRead: 'Durée de la session (min)',
      minutesReadHint: 'Facultatif — affine ton estimation en calibrant ta vitesse de lecture.',
      journalNote: 'Note pour le journal (facultatif)',
      startedAt: 'Commencé le',
      finishedAt: 'Fini le',
      remainingTitle: 'Temps de lecture restant',
      remainingUnknown: 'À estimer — indique le nombre de pages de ton édition.',
      speedCalibrated: 'calibrée sur tes sessions de lecture',
      speedDefault: 'moyenne par défaut (30 pages/h) — chronomètre une session pour l’affiner',
      speedUnit: 'pages/h',
      editionTitle: 'Mon édition',
      pagesTotal: 'Nombre de pages',
      pagesHint: 'Prérempli avec la médiane des éditions Open Library — mets ta valeur.',
      pagesAuto: 'médiane des éditions',
      pagesManual: 'ta valeur',
      pagesReset: 'Revenir à la médiane',
      isbn: 'ISBN',
      pagesShort: 'p.',
    },
    home: {
      inProgress: 'En ce moment',
      resumeHint: 'Reprendre :',
      emptyTitle: 'Rien en cours',
      empty: 'Reprends un titre déjà commencé, ou ajoute-en un nouveau.',
      seeLibrary: 'Voir toute la bibliothèque →',
      // Le héros de l'accueil : l'œuvre la plus proche d'être finie.
      heroEyebrow: 'À reprendre',
      heroResume: 'Reprendre',
      heroRemaining: 'restantes',
      heroRemainingUnknown: 'Durée à estimer',
      alsoInProgress: 'Aussi en cours',
    },
    budget: {
      title: 'Ton budget temps',
      totalPrefix: 'Pour tout finir',
      estimatedNote: 'Certaines durées sont estimées ou inconnues — affine-les depuis tes fiches.',
      games: 'Jeux',
      series: 'Séries',
      films: 'Films',
      books: 'Livres',
      inProgressLabel: 'en cours',
      backlogLabel: 'backlog',
      wishlistLabel: 'envies',
      toWatchLabel: 'à voir',
      toReadLabel: 'à lire',
      remaining: 'restant',
      remainingSuffix: 'restantes',
      unknown: 'durée à estimer',
      estimateMark: 'estimation',
      empty: 'Ajoute des titres à ta bibliothèque pour voir ton budget temps.',
    },
  },
} as const;
